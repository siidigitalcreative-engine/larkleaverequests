...
import { sendChangeOffDecisionCardEnhanced } from "@/lib/decision-notifications";
...
function dateText(value: unknown) {
  ...
  return new Intl.DateTimeFormat("en-PH", { timeZone: "Asia/Manila", year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date(timestamp));
}
...
export async function GET(request: Request, { params }: { params: { recordId: string } }) {
  try {
    ...
    return NextResponse.json({ request: {
      ...
      requestId: text(f["Change Off Request ID"]) || text(f["Request ID"]),
      ...
      currentOffDate: numberValue(f["Current Off-Date"]),
      requestedNewOffDate: numberValue(f["Requested New Off-Date"]),
      reason: text(f["Reason for Change"]) || text(f["Reason"]),
...
export async function POST(request: Request, { params }: { params: { recordId: string } }) {
  try {
    ...
    await updateChangeOffDecision({ recordId: params.recordId, decision, rejectionReason, approvalComment });
    ...
    try {
      await sendChangeOffDecisionCardEnhanced({
        approvalGroup: text(f["Approval Group"]),
        employeeName: text(f["Employee Name"]),
        requestId: text(f["Change Off Request ID"]) || text(f["Request ID"]),
